...
def far_cellid(point: Point, cells: pd.DataFrame, far_distance: int) -> Optional[float]:
...
def neighbor_cellid(cellid: int, celllist: Optional[List[int]]) -> int:
    ...
    while not neighbor_cell and stop_counter < 200:
        ...
        four_neighbors = list(
            set(
                [cell.next() for cell in four_neighbors]
                + [cell.prev() for cell in four_neighbors]
                + [cell.GetEdgeNeighbors()[2] for cell in four_neighbors]
                + [cell.GetEdgeNeighbors()[1] for cell in four_neighbors]
            )
        )
...
def get_s2cover_for_s2polygon(s2polygon: s2.S2Polygon, level: int) -> Optional[List]:
...
def plot_cells(cells: s2.S2Cell, location: Sequence[Point], zoom_level: int):
    ...
    # Create a map.
    map_osm = folium.Map(location=location, zoom_start=zoom_level, tiles="Stamen Toner")
...
def get_centers_from_s2cellids(s2cell_ids: Sequence[int64]) -> Sequence[Point]:
...
def get_center_from_s2cellids(s2cell_ids: Sequence[int64]) -> Sequence[CoordsYX]:
    """Returns the center latitude and longitude of s2 cells.
    Arguments:
      s2cell_ids: array of valid s2 cell ids. 1D array
    Returns:
...
def get_distance_between_points(point_1: Point, point_2: Point) -> int:
    ...
    dist = ox.distance.great_circle_vec(point_1.y, point_1.x, point_2.y, point_2.x)
