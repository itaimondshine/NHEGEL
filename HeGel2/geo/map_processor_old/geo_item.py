...
from HeGel2.geo.map_processor_old import osm
...
class GeoLandmark:
    ...
    def __attrs_post_init__(self):
        ...
        for k, v in self.pivot_gdf.to_dict().items():
            ...
            if isinstance(v, str) and v and k not in NOT_DISPLAY_TAGS:
                if not (
                    self.landmark_type in ["end_point", "near_pivot", "beyond_pivot"]
                    and "name" in k
                ):
                    landmark_dict[k.replace("_", " ")] = v.replace("_", " ")

        landmark_desc_list = [f"{t}: {v}" for t, v in landmark_dict.items()]
    ...
    def to_rvs_format(self):
        ...
        centroid = (
            util.tuple_from_point(self.geometry.centroid) if self.geometry else None
        )

        return [self.landmark_type, self.osmid, self.main_tag, centroid]

    @classmethod
    def create_from_pivot(cls, pivot: gpd.GeoDataFrame, pivot_type: str):
        """Construct a GeoLandmark."""
        return GeoLandmark(
            pivot_type, pivot["osmid"], pivot["geometry"], str(pivot["main_tag"]), pivot
        )
...
def save(entities: Sequence[GeoEntity], path_to_save: str):
    ...
    for entity in entities:
        for pivot_type, pivot in entity.geo_landmarks.items():
            geo_types_all[pivot_type] = geo_types_all[pivot_type].append(
                pivot.pivot_gdf
            )
        ...
        geo_types_all["path_features"] = geo_types_all["path_features"].append(
            features_gdf
        )
